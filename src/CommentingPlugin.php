...
class CommentingPlugin extends Omeka_Plugin_AbstractPlugin
{
    ...
    public function hookInstall()
    {
        ...
        $sql = "
            CREATE TABLE IF NOT EXISTS `$db->Comment` (
              ...
              `flagged` decimal(5,5) NOT NULL DEFAULT '0',
    ...
    public function hookUpgrade($args)
    {
        ...
        if(version_compare($old, '2.0', '<')) {
            $sql = "ALTER TABLE `$db->Comment` ADD `flagged` DECIMAL(5,5) NOT NULL DEFAULT '0' AFTER `approved` ";
            $db->query($sql);
        }
        ...
        if(version_compare($old, '2.1', '<')) {
            ...
            set_option('commenting_reqapp_comment_roles', serialize(array()));
            $sql = "ALTER TABLE `$db->Comment` CHANGE `flagged` `flagged` DECIMAL(5,5) NOT NULL DEFAULT '0'";
            $db->query($sql);
        }
    }
